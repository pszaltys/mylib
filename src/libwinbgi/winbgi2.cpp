...
inline int x_scale(double x) { return (int)(x*scale_x + scale_dx); }
inline int y_scale(double y) {
	if (scale_y_log) {
		return (int)(log10(y)*scale_y + scale_dy);
	}
	else {
		return (int)(y*scale_y + scale_dy);
	}
}
...
{
	int r=(int)rd, g=(int)gd, b=(int)bd;
...
void circle(double dx, double dy, int radius)
{
	int x = x_scale(dx);
	int y = y_scale(dy);
...
void circle_s(double dx, double dy, double radx, double rady)
{
	int x=x_scale(dx);
	int y=y_scale(dy);

    pcache.select(color+BG); 
    int ry = (int) (rady * scale_y);
    int rx = (int) (radx * scale_x);
    if (bgiemu_handle_redraw || visual_page != active_page) { 
	Arc(hdc[1], x-rx, y-ry, x+rx, y+ry, x+rx, y, x+rx, y);
    }
    if (visual_page == active_page) { 
...
{
	putpixel(x_scale(x),y_scale(y),1);
}
...
void all_scale(double x0, double y0, double x1, double y1, int y_log) {
	...
	line(sx-mag,my+mag,mx+mag,my+mag);
	{
		...
		settextjustify(1, 2);
		//, TA_BASELINE, TA_TOP};
		sprintf_s(xstr,"%.1lg",(x1-x0)/10.);
		sscanf_s(xstr,"%lf",&dx);
		double x=-floor(-x0/dx)*dx;
		for (int i = 0; i<=floor((x1-x0)/dx); i++) {
			int tick;
			char xstr[100];
			sprintf_s(xstr,"%.2lg",x);
			tick = (int)((x-x0)*(mx-sx)/(x1-x0));
	...
	}
	{
		...
		for (int i = 0; i<=floor((y1-y0)/dy); i++) {
			...
			char ystr[100];
			if (y_log) {
				sprintf_s(ystr,"10^%.2lg",y);
			} else {
				sprintf_s(ystr,"%.2lg",y);
			}
			tick = (int) ((y-y0)*(my-sy)/(y1-y0));
